...
function swipeRightService() {
	if(sliderService.clientWidth > (serviceContainerWidth + 80)) {
		if (serviceContainerWidth > 628 && left <= (sliderService.clientWidth - 1060) && left < 0) {
			left = left + 340;
		}else if (serviceContainerWidth == 628 && left > (628 - sliderService.clientWidth) && left < 0) {
			left = left + 314;
		}else if (serviceContainerWidth == 300 && left > (300 - sliderService.clientWidth) && left < 0) {
			left = left + 300;
			...
			left = (serviceContainerWidth - sliderService.clientWidth) - 14;
...
function swipeRightContent() {
	if(slideContent.clientWidth > (contentContainerWidth + 80)) {
		if (contentContainerWidth > 628 && left < (slideContent.clientWidth - 1060) && left < 0) {
			left = left + 340;
		}else if (contentContainerWidth == 628 && left > (628 - slideContent.clientWidth) && left < 0) {
			left = left + 314;
		}else if (contentContainerWidth == 300 && left > (300 - slideContent.clientWidth) && left < 0) {
			left = left + 300;
		}else if (slideContent.style.left == 0 || slideContent.style.left == '0px') {
